...
- Base de dados: schema **36 → 37**
- Export/restore: snapshot **32 → 33**
...
- `export_snapshot.dart` — versão máxima **33**; campos `google_timeline_import` e `google_timeline_place_labels` só em v33+.
...
- `schemaVersion` **37**; migração `from < 37`.
...
- Export v33; banda de export **1..33** (rejeita 34).
- Migração v36→v37: segundo import substitui visitas e **mantém** o rótulo.
- Widget: hub mostra CTA de import na aba Dia; aba Viagens ainda lista viagens manuais.
- Expectativas de versão de export nos testes de repositório/restore atualizadas para 33.
