...
class TimelineFrequentPlace extends Equatable {
  const TimelineFrequentPlace({this.placeId, this.location, this.label});
...
class GoogleTimelineDocument extends Equatable {
  ...
  factory GoogleTimelineDocument.fromJson(Map<String, dynamic> json) {
    ...
    Iterable<Map<String, dynamic>> maps(Object? raw) sync* {
    ...
    return GoogleTimelineDocument(
      visits: [
        for (final item in maps(json['visits']))
          TimelineVisit(
            ...
            candidateProbability: (item['candidate_probability'] as num?)
                ?.toDouble(),
      ...
      activities: [
        for (final item in maps(json['activities']))
          TimelineActivity(
            startAt: dt(item['start_at']),
            endAt: dt(item['end_at']),
            startOffsetMinutes: (item['start_offset'] as num?)?.toInt(),
      ...
      paths: [
        for (final item in maps(json['paths']))
          TimelinePathSegment(
            startAt: dt(item['start_at']),
            endAt: dt(item['end_at']),
            startOffsetMinutes: (item['start_offset'] as num?)?.toInt(),
            endOffsetMinutes: (item['end_offset'] as num?)?.toInt(),
            points: [
              for (final p in maps(item['points']))
      ...
      trips: [
        for (final item in maps(json['trips']))
          TimelineMemoryTrip(
            startAt: dt(item['start_at']),
            endAt: dt(item['end_at']),
            distanceFromOriginKms: (item['distance_from_origin_kms'] as num?)
                ?.toInt(),
            destinationPlaceIds: [
              for (final id
                  in (item['destination_place_ids'] as List? ?? const []))
                id.toString(),
            ],
          ),
      ],
      notes: [
        for (final item in maps(json['notes']))
          TimelineMemoryNote(
            startAt: dt(item['start_at']),
            endAt: dt(item['end_at']),
      ...
      frequentPlaces: [
        for (final item in maps(json['frequent_places']))
      ...
      affinities: [
        for (final item in maps(json['affinities']))
      ...
      positions: [
        for (final item in maps(json['positions']))
          TimelineRawPosition(
            ...
            speedMetersPerSecond: (item['speed_meters_per_second'] as num?)
                ?.toDouble(),
          ),
      ],
      sensorActivities: [
        for (final item in maps(json['sensor_activities']))
          TimelineSensorActivity(
            timestamp: dt(item['timestamp']),
            activities: [
              for (final a in maps(item['activities']))
                (
